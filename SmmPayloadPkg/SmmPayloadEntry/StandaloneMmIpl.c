  ...
  )
{
  ...
  FvHeader = (EFI_FIRMWARE_VOLUME_HEADER *)0x800000;

  // Find SecCore file first
  Status = GetFfsFileByType (FvHeader, FileType, FileInstance, &FileHeader);
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }

  // Find PE32 or TE section in SecCore file
  Status = GetSectionByType (FileHeader, SectionType, SectionInstance, Buffer);
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }
  ...
  *Size = SectionSize - sizeof(EFI_PE32_SECTION);
  ...
  if (FileType == EFI_FV_FILETYPE_MM_CORE_STANDALONE) {
    gMmCorePrivate->StandaloneBfvAddress = (EFI_PHYSICAL_ADDRESS)(UINTN)FvHeader;
  }
